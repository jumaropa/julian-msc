...
res((uf,pf,up,pp),(vf,qf,vp,qp)) = a((uf,pf,up,pp),(vf,qf,vp,qp)) + apb((up,pp),(vp,qp))
jac((uf,pf,up,pp),(duf,dpf,dup,dpp),(vf,qf,vp,qp)) = a((duf,dpf,dup,dpp),(vf,qf,vp,qp)) + dapb(up,dup,vp)

op = FEOperator(res,jac,X,Y)

using LineSearches: BackTracking
nls = NLSolver(show_trace=true, method=:newton, linesearch=BackTracking(), iterations=25)

## solve
uhf, phf, uhp, php = solve(nls, op)
writevtk(Ω_f,"twodomain-results-f", cellfields=["uhf" => uhf, "phf" => phf])
writevtk(Ω_p,"twodomain-results-p", cellfields=["uhp" => uhp, "php" => php])
