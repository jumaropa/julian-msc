...
add_tag_from_tags!(labels_Ω,"bottom",[5])      # assign the label "bottom" to the entity 1,2 and 5 (bottom corners and bottom side)
...
### Bilinear Forms
a((u,p),(v,q)) = ∫(∂t(u)⋅v + 1/Re * ∇(u)⊙∇(v) - p*(∇⋅v) + q*(∇⋅u))dΩ    #Stokes
b((u,p),(v,q)) = ∫(v⊙(Gr_v*u) + v⊙(Gr_i*((u⋅u).^(1/2))*u))dΩ_p          #Darcy-Forchheimer
c(u,v) = ∫(v⋅((∇(u)')⋅u))dΩ                                              #convective term
l((v,q)) = ∫(0*q)dΩ

#residual
res(t,(u,p),(v,q)) = a((u,p),(v,q)) + b((u,p),(v,q)) + c(u,v)# - l((v,q))
#res(t,(u,p),(v,q)) = a((u,p),(v,q)) + b((u,p),(v,q)) - l((v,q))
...
createpvd("channel_flow_transient_solution") do pvd
  for ((uh,ph),t) in xₕₜ
    println(t)
    pvd[t] = createvtk(Ω,"results-PE-lag-conv-trans_$t"*".vtu",cellfields=["uh"=>uh, "ph"=>ph], order=1)
  end
end
