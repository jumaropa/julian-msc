...
function main(n_x,n_y)
  ...
  Y = MultiFieldFESpace([Vf,Vp,Q]) #test
  X = MultiFieldFESpace([Uf,Up,P]) #trial
  ...
  ## Weak Form ##
  a((uf,up),(vf,vp)) = ∫( μ*(∇(uf)⊙∇(vf)) + 1/Kf*(uf⋅vf) + 1/Kd*(up⋅vp) )dΩ
  b_((uf,up),q) = ∫( ∇(q)⋅(uf+up) )dΩ - ∫( jump(q*nΛ)⋅mean(uf) )dΛ - ∫( jump(q*nΛ)⋅mean(up) )dΛ -
                  ∫( (q*n_Γ_f_out)⋅uf )dΓ_f_out - ∫( (q*n_Γ_p_out)⋅up )dΓ_p_out
  b(p,(vf,vp)) = ∫( p*(∇⋅vf) + p*(∇⋅vp) )dΩ
  res((uf,up,p),(vf,vp,q)) = a((uf,up),(vf,vp)) + b_((uf,up),q) - b(p,(vf,vp))
  jac((uf,up,p),(duf,dup,dp),(vf,vp,q)) = a((duf,dup),(vf,vp)) + b_((duf,dup),q) - b(dp,(vf,vp))
  op = FEOperator(res,jac,X,Y)

  nls = NLSolver(show_trace=true, method=:newton, linesearch=BackTracking(), iterations=10)

  ## solve
  uhf, uhp, ph = solve(nls, op)
